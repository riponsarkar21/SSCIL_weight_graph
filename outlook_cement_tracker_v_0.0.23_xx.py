...
class CementDeliveryTracker:
    ...
    def plot_chart(self, parent_frame, from_date, to_date):
        ...
        if self.show_short_var.get():
            line, = ax.plot(df['date'], df['short'], 'o-', color='#ef4444', linewidth=2, markersize=6, label='Daily Short (KG)', picker=5)
            lines.append(('Short', line, df['short'], 'KG'))
        
        if self.show_excess_var.get():
            line, = ax.plot(df['date'], df['excess'], 's-', color='#10b981', linewidth=2, markersize=6, label='Daily Excess (KG)', picker=5)
            lines.append(('Excess', line, df['excess'], 'KG'))
        ...
        if self.show_perbag_var.get() or self.show_bagweight_var.get():
            ...
            if self.show_perbag_var.get():
                line, = ax2.plot(df['date'], df['per_bag_short_excess'], '^-', color='#3b82f6', linewidth=2, markersize=6, label='Per Bag S/E', picker=5)
                lines.append(('Per Bag S/E', line, df['per_bag_short_excess'], ''))
            
            if self.show_bagweight_var.get():
                line, = ax2.plot(df['date'], df['bag_weight'], 'd-', color='#8b5cf6', linewidth=2, markersize=6, label='Bag Weight (KG)', picker=5)
                lines.append(('Bag Weight', line, df['bag_weight'], 'KG'))
        ...
        def on_hover(event):
            nonlocal tooltip, tooltip_frame
            ...
            for line_name, line, data_series, unit in lines:
                if line.contains(event)[0]:
                    ...
                    for i, (x, y) in enumerate(zip(xdata, ydata)):
                        ...
                        display_x, display_y = line.axes.transData.transform((x, y))
                        distance = ((display_x - event.x) ** 2 + (display_y - event.y) ** 2) ** 0.5
                        
                        if distance < min_distance and distance < 20:  # 20 pixel threshold
                            min_distance = distance
                            closest_point = i
                            point_info = (line_name, x, y, unit, i)
            
            if point_info and closest_point is not None:
                line_name, x, y, unit, idx = point_info
                
                # Get all data for this date
                date_val = df.iloc[idx]['date']
                short_val = df.iloc[idx]['short']
                excess_val = df.iloc[idx]['excess']
                perbag_val = df.iloc[idx]['per_bag_short_excess']
                bagweight_val = df.iloc[idx]['bag_weight']
                ...
                tooltip_frame = tk.Frame(parent_frame, bg='#1f2937', relief=tk.RAISED, borderwidth=2)
                tooltip_frame.place(x=event.x + 10, y=event.y + 10)
                ...
                # Separator
                separator = tk.Frame(tooltip_frame, bg='#4b5563', height=1)
                separator.pack(fill=tk.X, padx=5)
                
                # Data rows
                data_items = [
                    ('🔴 Short:', f'{int(short_val):,} KG', '#ef4444'),
                    ('🟢 Excess:', f'{int(excess_val):,} KG', '#10b981'),
                    ('🔵 Per Bag S/E:', f'{perbag_val:.4f}', '#3b82f6'),
                    ('🟣 Bag Weight:', f'{bagweight_val:.3f} KG', '#8b5cf6')
                ]
                
                for label, value, color in data_items:
                    row_frame = tk.Frame(tooltip_frame, bg='#1f2937')
                ...
                # Bottom padding
                tk.Label(tooltip_frame, text='', bg='#1f2937', pady=2).pack()
