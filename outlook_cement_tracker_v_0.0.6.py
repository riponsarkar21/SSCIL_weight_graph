    ...
    def parse_email_body(self, body):
        ...
        try:
            ...
            if not daily_report_match:
                print(f"    ✗ Failed: Could not find 'Daily Report' section")
                return None
            
            print(f"    Found 'Daily Report' section")
            ...
            if monthly_match:
                ...
                print(f"    Isolated Daily Report section (before Monthly report)")
            
            # Debug: Print a sample of the daily section
            print(f"    Daily section preview: {daily_section[:200]}...")
            
            # Now we need to find the row with Short and Excess values
            # Look for the data row with: Total Delivery, Bag Weight, Physical Weight, Short, Excess
            
            # First, try to find the header line
            header_match = re.search(r'Short\s+Excess', daily_section)
            if not header_match:
                print(f"    ✗ Failed: Could not find 'Short Excess' header")
                return None
            
            print(f"    Found 'Short Excess' header")
            
            # Get text after header
            after_header = daily_section[header_match.end():]
            
            # Look for the data row - it should have 5 numbers in sequence
            # Pattern: Total Delivery, Bag Weight, Physical Weight, Short, Excess
            # The numbers might be separated by whitespace or tabs
            ...
            if not row_match:
                ...
                row_match = re.search(r'(\d+)\s*\n?\s*(\d+)\s*\n?\s*(\d+)\s*\n?\s*(\d+)\s*\n?\s*(\d+)', after_header)
                
            if not row_match:
                print(f"    ✗ Failed: Could not parse data row")
                print(f"    After header text: {after_header[:300]}")
                return None
            
            # Extract values
            total_delivery = int(row_match.group(1))
            bag_weight = int(row_match.group(2))
            physical_weight = int(row_match.group(3))
