...
class CementDeliveryTracker:
    ...
    def parse_email_body(self, body):
        ...
        try:
            ...
            # Extract Short and Excess from Daily Report section
            # Look for pattern like: Short  Excess followed by numbers
            short_excess_match = re.search(r'Short\s+Excess[\s\S]*?(\d+)\s+(\d+)', body)
            
            if not short_excess_match:
                return None
            
            short = int(short_excess_match.group(1))
            excess = int(short_excess_match.group(2))
